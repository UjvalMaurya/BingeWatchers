...
export default function AddWebSeries() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/content`, {
        ...
        body: JSON.stringify({ ...form, type: "Web Series" }),
      });

      const data = await res.json();

      if (res.ok) {
        toast.success("Web Series Added Successfully!", { autoClose: 2000 });
        setForm({
          title: "",
          poster: "",
          rating: "",
        ...
        setTimeout(() => navigate("/view/webseries"), 2000);
      } else {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6">
      ...
      <h1 className="text-3xl font-bold mb-6 text-center">Add Web Series</h1>
          ...
          {["prequel", "sequel", "spinOff", "sharedUniverse"].map((field) => (
            <div key={field} className="mb-3">
              ...
              <div className="flex flex-wrap gap-2">
                {allContent
                  .filter((c) =>
                    field === "prequel" || field === "sequel" ? c.type === "Web Series" : true
                  )
                  .map((c) => (
                    <button
                      type="button"
                      ...
                      className={`px-2 py-1 rounded text-sm border ${
                        form.connections[field].includes(c._id)
